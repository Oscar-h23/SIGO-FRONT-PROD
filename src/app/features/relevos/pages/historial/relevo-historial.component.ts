...
export class RelevoHistorialComponent implements OnInit {
  ...
  async generarPdf(relevo: RelevoResponse): Promise<void> {
    ...
    try {
      const pdf=new jsPDF({unit:'mm',format:'a4'}); const margin=14; let y=14;
      ...
      await this.agregarEvidenciasPdf(pdf,relevo,logo);
  ...
  private encabezadoPdf(pdf:jsPDF,logo:string|null,relevo:RelevoResponse):void{
    pdf.setFillColor(15,23,42);pdf.rect(0,0,210,36,'F');
    if(logo){try{pdf.addImage(logo,'PNG',14,8,38,18,undefined,'FAST');}catch{}}
    pdf.setTextColor(255,255,255);pdf.setFont('helvetica','bold');pdf.setFontSize(16);pdf.text('REPORTE DE RELEVO DE TURNO',196,14,{align:'right'});
    pdf.setFont('helvetica','normal');pdf.setFontSize(8.5);pdf.setTextColor(203,213,225);pdf.text('SIGO · Sistema de Gestión Operativa',196,21,{align:'right'});pdf.text(`Plaza ${relevo.plazaCodigo} · Turno ${relevo.turnoCodigo}`,196,27,{align:'right'});
    pdf.setDrawColor(37,99,235);pdf.setLineWidth(1.2);pdf.line(14,36,196,36);pdf.setTextColor(15,23,42);
  }
  private bloqueDatos(pdf:jsPDF,r:RelevoResponse,y:number):number{
    pdf.setFillColor(248,250,252);pdf.setDrawColor(226,232,240);pdf.roundedRect(14,y,182,31,3,3,'FD');
    const datos=[['PLAZA',`${r.plazaCodigo}${r.plazaDescripcion?' · '+r.plazaDescripcion:''}`],['FECHA',r.fecha],['HORA',this.horaCorta(r.hora)],['TURNO',r.turnoCodigo],['REGISTRADO POR',r.operadorNombre]];
    const xs=[20,78,112,143,164]; const widths=[52,28,25,17,27];
    datos.forEach((d,i)=>{pdf.setFont('helvetica','bold');pdf.setFontSize(6.8);pdf.setTextColor(100,116,139);pdf.text(d[0],xs[i],y+9);pdf.setFontSize(8.8);pdf.setTextColor(15,23,42);const lines=pdf.splitTextToSize(d[1],widths[i]);pdf.text(lines,xs[i],y+16);});
    return y+40;
  }
  private tituloSeccion(pdf:jsPDF,titulo:string,y:number):void{pdf.setFillColor(239,246,255);pdf.roundedRect(14,y,182,8,2,2,'F');pdf.setFont('helvetica','bold');pdf.setFontSize(9);pdf.setTextColor(29,78,216);pdf.text(titulo,18,y+5.4);pdf.setTextColor(15,23,42);}
  private dibujarSeccion(pdf:jsPDF,titulo:string,items:RelevoChecklistResponse[],y:number,logo:string|null,r:RelevoResponse):number{
    y=this.ensureSpace(pdf,y,20,logo,r);this.tituloSeccion(pdf,titulo,y);y+=12;
    for(const item of items){const detalle=item.detalle?item.detalle:'Sin observaciones';const cant=item.cantidad!==null?` · Cantidad: ${item.cantidad}`:'';const lines=pdf.splitTextToSize(detalle,118);const h=Math.max(13,8+lines.length*3.5);y=this.ensureSpace(pdf,y,h+3,logo,r);pdf.setDrawColor(226,232,240);pdf.roundedRect(14,y,182,h,2,2,'S');pdf.setFont('helvetica','bold');pdf.setFontSize(8.5);pdf.text(`${item.nombre}${cant}`,18,y+5.5);this.badgeEstado(pdf,item.estado,190,y+5.5);pdf.setFont('helvetica','normal');pdf.setFontSize(7.5);pdf.setTextColor(71,85,105);pdf.text(lines,18,y+10.5);pdf.setTextColor(15,23,42);y+=h+3;} return y+3;
  }
  private dibujarVias(pdf:jsPDF,vias:RelevoViaResponse[],y:number,logo:string|null,r:RelevoResponse):number{
    y=this.ensureSpace(pdf,y,20,logo,r);this.tituloSeccion(pdf,'REPORTE DE VÍAS',y);y+=12;
    for(const v of vias){const detalle=v.detalle||'Sin observaciones';const lines=pdf.splitTextToSize(detalle,118);const h=Math.max(13,8+lines.length*3.5);y=this.ensureSpace(pdf,y,h+3,logo,r);pdf.setDrawColor(226,232,240);pdf.roundedRect(14,y,182,h,2,2,'S');pdf.setFont('helvetica','bold');pdf.setFontSize(8.5);pdf.text(`Vía ${v.numero}${v.nombre?' · '+v.nombre:''}`,18,y+5.5);this.badgeEstado(pdf,v.estado,190,y+5.5);pdf.setFont('helvetica','normal');pdf.setFontSize(7.5);pdf.setTextColor(71,85,105);pdf.text(lines,18,y+10.5);pdf.setTextColor(15,23,42);y+=h+3;}return y+3;
  }
  private badgeEstado(pdf:jsPDF,estado:string,x:number,y:number):void{const label=this.estadoLabel(estado);const w=estado==='NO_OPERATIVO'?25:estado==='OBSERVADO'?20:18;if(estado==='OPERATIVO'){pdf.setFillColor(220,252,231);pdf.setTextColor(21,128,61);}else if(estado==='OBSERVADO'){pdf.setFillColor(254,243,199);pdf.setTextColor(180,83,9);}else{pdf.setFillColor(254,226,226);pdf.setTextColor(185,28,28);}pdf.roundedRect(x-w,y-4.3,w,6,2,2,'F');pdf.setFont('helvetica','bold');pdf.setFontSize(6.5);pdf.text(label,x-w/2,y,{align:'center'});pdf.setTextColor(15,23,42);}
  private cajaTexto(pdf:jsPDF,label:string,texto:string,y:number):number{const lines=pdf.splitTextToSize(texto,170);const h=11+lines.length*4;pdf.setFillColor(248,250,252);pdf.setDrawColor(226,232,240);pdf.roundedRect(14,y,182,h,2,2,'FD');pdf.setFont('helvetica','bold');pdf.setFontSize(7.5);pdf.setTextColor(100,116,139);pdf.text(label.toUpperCase(),18,y+5);pdf.setFont('helvetica','normal');pdf.setFontSize(8.5);pdf.setTextColor(30,41,59);pdf.text(lines,18,y+10);return y+h+4;}
  private ensureSpace(pdf:jsPDF,y:number,needed:number,logo:string|null,r:RelevoResponse):number{if(y+needed<=278)return y;pdf.addPage();this.encabezadoPdf(pdf,logo,r);return 46;}

  private async agregarEvidenciasPdf(pdf:jsPDF,r:RelevoResponse,logo:string|null):Promise<void>{
    const ev:{titulo:string;subtitulo:string;url:string}[]=[];for(const i of r.checklist)for(const e of i.evidencias??[])ev.push({titulo:i.nombre,subtitulo:this.estadoLabel(i.estado),url:e.urlArchivo});for(const v of r.vias)for(const e of v.evidencias??[])ev.push({titulo:`Vía ${v.numero}`,subtitulo:this.estadoLabel(v.estado),url:e.urlArchivo});if(!ev.length)return;
    for(let i=0;i<ev.length;i+=4){pdf.addPage();this.encabezadoPdf(pdf,logo,r);this.tituloSeccion(pdf,'EVIDENCIAS FOTOGRÁFICAS',46);const lote=ev.slice(i,i+4);for(let j=0;j<lote.length;j++){const col=j%2,row=Math.floor(j/2);const x=25+col*92,y=60+row*108;pdf.setFont('helvetica','bold');pdf.setFontSize(8.5);pdf.setTextColor(15,23,42);pdf.text(lote[j].titulo,x,y);pdf.setFont('helvetica','normal');pdf.setFontSize(7);pdf.setTextColor(100,116,139);pdf.text(lote[j].subtitulo,x,y+4);const data=await this.imagenDataUrl(lote[j].url);const w=54,h=72;if(data){try{pdf.addImage(data,'JPEG',x,y+8,w,h,undefined,'FAST');}catch{}}pdf.setDrawColor(203,213,225);pdf.roundedRect(x,y+8,w,h,2,2,'S');}}
  }
  ...
  private async assetDataUrl(path:string):Promise<string|null>{try{const res=await fetch(path);if(!res.ok)return null;return await this.blobDataUrl(await res.blob());}catch{return null;}}
  private async imagenDataUrl(url:string):Promise<string|null>{try{const res=await fetch(url);if(!res.ok)return null;return await this.blobDataUrl(await res.blob());}catch{return null;}}
  private blobDataUrl(blob:Blob):Promise<string|null>{return new Promise(resolve=>{const reader=new FileReader();reader.onload=()=>resolve(typeof reader.result==='string'?reader.result:null);reader.onerror=()=>resolve(null);reader.readAsDataURL(blob);});}
